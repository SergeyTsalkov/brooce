...
var BrooceResourceDir = os.Getenv("BROOCE_RESOURCEDIR")
...
func init() {
	...
	if  BrooceResourceDir != "" {
		log.Println("ResourceDir:", BrooceResourceDir)
	}
	if  BrooceLogDir != "" {
...
func initDefaultDirs() {
	...
	if BrooceResourceDir == "" {
		BrooceResourceDir = filepath.Join(BrooceConfigDir, "resources")
	}

	// use local resources only if exists
	if !util.IsDir(BrooceResourceDir) {
		BrooceResourceDir = ""
	}
